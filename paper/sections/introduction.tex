...
This paper studies that mechanism in a controlled setting and then attacks it with a CPU-light standard-control benchmark tier. We deliberately separate the internal/proxy return $\score$ from real utility $\utility$, train a small learned Decision Transformer-style policy on synthetic offline trajectories with limited high-return support, and evaluate top-score selection under in-support, edge-support, and out-of-support return prompts. We then repeat the failure-and-repair question under CartPole-v1 dynamics. The goal is not benchmark dominance. The goal is a precise account of when selected return is evidence of utility and when it is a return-conditioning fantasy.
...
Fourth, the expanded suite pushes the candidate-count tail to $N=256$, sweeps target overshoot beyond the offline $q_{95}$ boundary, varies pilot-label budget, and stresses rollout noise. At $N=256$, selected proxy return increases by $0.224$ over $N=1$ while real utility decreases by $0.629$.
Fifth, we add a CartPole-v1 benchmark tier. Raw high-$N$ proxy selection improves the proxy but collapses true episode return, while support filtering, conservative penalties, behavior cloning, and the oracle diagnostic expose the missing support mechanism.
Finally, we include a claim audit that treats the study as a controlled v4 artifact: it supports the mechanism claims above, checks the expanded stress suite, checks the CartPole benchmark, checks the final PDF page count, and forbids stale or overbroad claims.
